...
add_keybind() {
    ...
    # Replace rather than skip, so an upgrade that changes the bindings lands
    # instead of silently keeping the old block.
    if grep -q "agenttij:begin" "$zellij_config"; then
        sed -i '/agenttij:begin/,/agenttij:end/d' "$zellij_config"
    fi
    cat >>"$zellij_config" <<EOF

// agenttij:begin — safe to delete (regenerated by scripts/install.sh)
keybinds {
    shared_except "locked" {
        ...
        // Unfold the workspace sidebar after 'c' parked it. Zellij identifies a
        // plugin by url *and* configuration, so this repeats the configuration
        // from agenttij-workspace.kdl — change one, change both, or the
        // keybind will launch a second sidebar instead of talking to this one.
        bind "$fold_keybind" {
